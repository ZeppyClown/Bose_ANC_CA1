...
COUNTS={'opening':3,'science':6,'technology':8,'product':6,'market':4,'cim':2,'cycle':4,'conclusion':2,'sources':1}
with sync_playwright() as p:
    ...
    for state in states[-2::-1]:
        ...
        assert page.evaluate('currentHash()')=='#'+state,state
    page.goto(URL+'#cim-2');page.keyboard.press('n')
    ...
    print('PASS: 36 states, all chapter content/assets, forward/back keyboard navigation, cues, replay smoke check, mobile width, portrait layout, no JS errors.')
    browser.close()
